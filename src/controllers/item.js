...
exports.getItemsByCategory = async (req, res) => {
    try {
        const item = await ItemSchema.find({categoryId : req.params.id , isActive : true}); // Retrieve all items from the database
        if(!item) {
...
exports.createItem = async (req, res) => {
    ...
    try {
        const cateogry = await CategorySchema.findById(req.body.categoryId);
        if(!cateogry || cateogry.isActive === false) {
